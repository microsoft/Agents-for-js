 ...
 * - Response processing
 */
export class AssistantsPlanner<TState extends AITurnState = AITurnState> implements Planner<TState> {
  ...
  /**
     ...
     * @private
     * @param {AITurnState} state - The application Turn State.
  ...
  private async waitForRun (
    ...
    while (true) {
      ...
      const run = await this._client.beta.threads.runs.retrieve(threadID, runID)
  ...
  /**
     ...
     * @param {string} threadID - The ID of the thread.
     * @param {string} lastMessageID - The ID of the last message.
     * @returns {Promise<Plan>} A plan based on the messages.
     */
  private async generatePlanFromMessages (threadID: string, lastMessageID: string): Promise<Plan> {
    const messages = await this._client.beta.threads.messages.list(threadID)

    const lastMessageIndex = messages.data.findIndex((message) => message.id === lastMessageID)
  ...
  /**
     ...
     * @param {string} threadID - The ID of the thread.
     */
  private async blockOnInProgressRuns (threadID: string): Promise<void> {
    while (true) {
      const runs = await this._client.beta.threads.runs.list(threadID, { limit: 1 })
      ...
      await this.waitForRun(threadID, latestRun.id)
    }
  }
