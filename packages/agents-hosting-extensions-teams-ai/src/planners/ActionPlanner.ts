...
import { AITurnState, PromptResponse } from '../types'
 ...
 */
export type ActionPlannerPromptFactory<TState extends AITurnState = AITurnState> = (
  context: TurnContext,
  state: TState,
 ...
 * @template TState Optional. Type of application state.
 */
export interface ActionPlannerOptions<TState extends AITurnState = AITurnState> {
  /**
 ...
 * @template TState Optional. Type of application state.
 */
export class ActionPlanner<TState extends AITurnState = AITurnState> implements Planner<TState> {
  ...
  public async continueTask (context: TurnContext, state: TState, ai: AI<TState>): Promise<Plan> {
    ...
    if (result.status !== 'success') {
      throw result.error ?? new Error('ActionPlanner failed to complete prompt')
    }
  ...
  public async completePrompt<TContent = string>(
    ...
    const inputVariable = 'temp.input'

    // Create LLM client
    const client = new LLMClient<TContent>({
      model,
      template,
      history_variable: historyVariable,
