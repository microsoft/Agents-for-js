 ...
 */
export interface AIOptions<TState extends AITurnState> {
  /**
 ...
 * @template TState Type of the turn state.
 */
export interface ConfiguredAIOptions<TState extends AITurnState> {
  /**
 ...
 */
export class AI<TState extends AITurnState = AITurnState> {
  ...
  /**
     ...
     * @param {TurnContext} context Current turn context.
     * @param {TState} state Current turn state.
     * @param {number} startTime Optional. Time the AI system started running
     * @param {number} stepCount Optional. Number of steps that have been executed.
     * @returns {Promise<boolean>} True if the plan was completely executed, otherwise false.
     */
  public async run (context: TurnContext, state: TState, startTime?: number, stepCount?: number): Promise<boolean> {
    try {
      ...
      let plan: Plan | undefined =
                stepCount === 0 ? await this._options.moderator.reviewInput(context, state) : undefined
      ...
      for (let i = 0; i < plan.commands.length; i++) {
        // Check for timeout
        if (Date.now() - startTime! > max_time || ++stepCount! > max_steps) {
          completed = false
      ...
      if (completed && should_loop && this._options.allow_looping) {
        return await this.run(context, state, startTime, stepCount)
      }
