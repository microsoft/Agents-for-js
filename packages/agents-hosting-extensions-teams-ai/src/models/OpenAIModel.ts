...
export class OpenAIModel implements PromptCompletionModel {
  ...
  public async completePrompt (
    ...
    const maxInputTokens = template.config.completion.max_input_tokens
    const model =
    ...
    // Render prompt
    const result = await template.prompt.renderAsMessages(context, memory, functions, tokenizer, maxInputTokens)
    if (result.tooLong) {
      return this.returnTooLong(maxInputTokens, result.length)
    }
  ...
  private returnTooLong (maxInputTokens: number, length: number): PromptResponse<string> {
    return {
      status: 'too_long',
      input: undefined,
      error: new Error(
                `The generated chat completion prompt had a length of ${length} tokens which exceeded the max_input_tokens of ${maxInputTokens}.`
      )
    }
  }
