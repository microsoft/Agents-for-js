...
describe('SequenceAugmentation', () => {
  ...
  const missingActionDO = JSON.stringify({
    type: 'plan',
    commands: [
      {
        type: 'DO'
  ...
  const invalidActionDO = JSON.stringify({
    type: 'plan',
    commands: [
      {
        type: 'DO',
  ...
  const missingResponseSAY = JSON.stringify({
    type: 'plan',
    commands: [
      {
        type: 'SAY'
  ...
  const validSAYandDO = JSON.stringify({
    type: 'plan',
    commands: [
      {
        type: 'DO',
    ...
    it('should fail a response due to missing action in DO command', async () => {
      await adapter.sendTextToBot('test', async (context) => {
        ...
        const response = await augmentation.validateResponse(
          ...
          state,
          tokenizer,
          { status: 'success', message: { role: 'assistant', content: missingActionDO } },
          3
        )
        assert.notEqual(response, undefined)
        assert.equal(response.type, 'Validation')
    ...
    it('should fail a response due to invalid action in DO command', async () => {
      await adapter.sendTextToBot('test', async (context) => {
        ...
        const response = await augmentation.validateResponse(
          ...
          state,
          tokenizer,
          { status: 'success', message: { role: 'assistant', content: invalidActionDO } },
          3
        )
        assert.notEqual(response, undefined)
        assert.equal(response.type, 'Validation')
    ...
    it('should fail a response due to missing response in SAY command', async () => {
      await adapter.sendTextToBot('test', async (context) => {
        ...
        const response = await augmentation.validateResponse(
          ...
          state,
          tokenizer,
          { status: 'success', message: { role: 'assistant', content: missingResponseSAY } },
          3
        )
        assert.notEqual(response, undefined)
        assert.equal(response.type, 'Validation')
    ...
    it('should fail a response due to random command', async () => {
      await adapter.sendTextToBot('test', async (context) => {
        ...
        const response = await augmentation.validateResponse(
          ...
          state,
          tokenizer,
          { status: 'success', message: { role: 'assistant', content: randomResponse } },
          3
        )
        assert.notEqual(response, undefined)
        assert.equal(response.type, 'Validation')
    ...
    it('should return a valid response with DO and SAY', async () => {
      await adapter.sendTextToBot('test', async (context) => {
        ...
        const response: PromptResponse<string> = {
          status: 'success',
          message: { role: 'assistant', content: validSAYandDO }
        }
