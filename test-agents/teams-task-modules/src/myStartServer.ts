...
export interface StartServerOptions {
  ...
  // Directory of static files (e.g. webpage task module HTML) to serve without
  // requiring a JWT, since the Teams iframe loading these URLs does not send
  // the bot's Bearer token.
  staticDir?: string
}
...
export function myStartServer (agent: AgentApplication<TurnState<any, any>> | ActivityHandler, authConfigurationOrOptions?: AuthConfiguration | StartServerOptions) : express.Express {
  ...
  // Mount static assets BEFORE the JWT middleware so that URL-based task modules
  // (loaded by the Teams iframe without a Bearer token) can be served.
