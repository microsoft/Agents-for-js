...
4. **Token access in proactive handlers.** Inside a `continueConversation` handler, callers
   retrieve Graph (or other OAuth) tokens the same way as in a normal turn:
   ```typescript
   const { token } = await app.authorization.getToken(context, 'graph')
   ```
   This works because `Activity.getContinuationActivity()` sets `from: reference.user`, so
   `ctx.activity.from.id` is the correct user ID for the Bot Framework User Token Service lookup.

   **Known inefficiency — double token fetch when `autoSignInHandlers` is used.**
   The `autoSignInHandlers` pre-flight in `Proactive.continueConversation()` calls
   `authorization.getToken()` for each handler to verify the user has signed in, but discards
   the returned tokens. `AzureBotAuthorization.token()` only writes to its per-turn turn-state
   cache (`setContext`) during the sign-in flow, not in the plain `getToken` path. So when the
   handler calls `getToken` again, it is a cache miss and makes a second network call to the
   token service.

   The fix would be for `AzureBotAuthorization` to cache the result in turn state inside
   `token()` (not just during the sign-in flow), or to expose `setContext` so the proactive
   pre-flight can prime the cache. Until then, callers using `autoSignInHandlers` should be
   aware of the double round-trip.
